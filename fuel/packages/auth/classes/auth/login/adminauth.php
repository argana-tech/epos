...
/**
 * AdminAuth basic login driver
 *
 * @package     Fuel
 * @subpackage  Auth
 */
class Auth_Login_Adminauth extends \Auth_Login_Driver
{
	/**
	...
	{
		\Config::load('adminauth', true, true, true);

		// setup the remember-me session object if needed
		if (\Config::get('adminauth.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('adminauth.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('adminauth.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	/**
	 * @var  array  AdminAuth class config
	 */
	protected $config = array(
		'drivers' => array('group' => array('Admingroup')),
	...
	protected function perform_check()
	{
		...
		$login_hash  = \Session::get('admin_login_hash');
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
				$this->user = \DB::select_array(\Config::get('adminauth.table_columns', array('*')))
					->where('username', '=', $username)
					->where('removed', '=', 0)
					->from(\Config::get('adminauth.table_name'))
					->execute(\Config::get('adminauth.db_connection'))->current();
			}

			// return true when login was verified, and either the hash matches or multiple logins are allowed
			if ($this->user and (\Config::get('adminauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
		...
		$this->user = \Config::get('adminauth.guest_login', true) ? static::$guest_login : false;
		\Session::delete('admin_username');
		\Session::delete('admin_login_hash');
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('adminauth.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('adminauth.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('adminauth.table_columns', array('*')))
			->where('removed', '=', 0)
			->where_open()
...
//			->or_where('email', '=', $username_or_email)
	...
	public function login($username_or_email = '', $password = '')
	{
		...
		\Session::set('admin_username', $this->user['username']);
		\Session::set('admin_login_hash', $this->create_login_hash());
	...
	public function force_login($user_id = '')
	{
		...
		$this->user = \DB::select_array(\Config::get('adminauth.table_columns', array('*')))
			...
			->where_close()
			->from(\Config::get('adminauth.table_name'))
			->execute(\Config::get('adminauth.db_connection'))
			->current();
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$same_users = \DB::select_array(\Config::get('adminauth.table_columns', array('*')))
			->where('removed', '=', 0)
			->where('username', '=', $username)
//			->or_where('email', '=', $email)
		...
		$result = \DB::insert(\Config::get('adminauth.table_name'))
			->set($user)
			->execute(\Config::get('adminauth.db_connection'));
	...
	public function update_user($values, $username = null)
	{
		...
		$current_values = \DB::select_array(\Config::get('adminauth.table_columns', array('*')))
			->where('username', '=', $username)
			->from(\Config::get('adminauth.table_name'))
			->execute(\Config::get('adminauth.db_connection'));
		...
		$affected_rows = \DB::update(\Config::get('adminauth.table_name'))
			->set($update)
			->where('username', '=', $username)
			->execute(\Config::get('adminauth.db_connection'));
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('adminauth.table_name'))
			->where('username', '=', $username)
			->execute(\Config::get('adminauth.db_connection'));
	...
	public function create_login_hash()
	{
		...
		//$login_hash = sha1(\Config::get('adminauth.login_hash_salt').$this->user['username'].$last_login);
		$login_hash = sha1(\Config::get('adminauth.login_hash_salt').$this->user['username']);

		\DB::update(\Config::get('adminauth.table_name'))
	...
	public function get_groups()
	{
		...
		return array(array('Admingroup', $this->user['group']));
	}
	...
	{
		return \Config::get('adminauth.guest_login', true);
	}
}
