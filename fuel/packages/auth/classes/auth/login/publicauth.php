...
/**
 * PublicAuth basic login driver
 *
 * @package     Fuel
 * @subpackage  Auth
 */
class Auth_Login_Publicauth extends \Auth_Login_Driver
{
	/**
	...
	{
		\Config::load('publicauth', true, true, true);

		// setup the remember-me session object if needed
		if (\Config::get('publicauth.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('publicauth.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('publicauth.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	/**
	 * @var  array  PublicAuth class config
	 */
	protected $config = array(
		'drivers' => array('group' => array('Publicgroup')),
	...
	protected function perform_check()
	{
		...
		$username    = \Session::get('public_username');
		$login_hash  = \Session::get('public_login_hash');
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			{
				$this->user = \DB::select_array(\Config::get('publicauth.table_columns', array('*')))
					->where('username', '=', $username)
					->where('removed', '=', 0)
					->from(\Config::get('publicauth.table_name'))
					->execute(\Config::get('publicauth.db_connection'))->current();
			}

			// return true when login was verified, and either the hash matches or multiple logins are allowed
			if ($this->user and (\Config::get('publicauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
		...
		$this->user = \Config::get('publicauth.guest_login', true) ? static::$guest_login : false;
		\Session::delete('public_username');
		\Session::delete('public_login_hash');
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('publicauth.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('publicauth.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('publicauth.table_columns', array('*')))
			->where('removed', '=', 0)
			->where_open()
...
//			->or_where('email', '=', $username_or_email)
	...
	public function login($username_or_email = '', $password = '')
	{
		...
		\Session::set('public_username', $this->user['username']);
		\Session::set('public_login_hash', $this->create_login_hash());
	...
	public function force_login($user_id = '')
	{
		...
		$this->user = \DB::select_array(\Config::get('publicauth.table_columns', array('*')))
			...
			->where_close()
			->from(\Config::get('publicauth.table_name'))
			->execute(\Config::get('publicauth.db_connection'))
			->current();
	...
	public function create_user($username, $password, $email, $group = 1, Array $profile_fields = array())
	{
		...
		$same_users = \DB::select_array(\Config::get('publicauth.table_columns', array('*')))
			->where('removed', '=', 0)
			->where('username', '=', $username)
//			->or_where('email', '=', $email)
		...
		$result = \DB::insert(\Config::get('publicauth.table_name'))
			->set($user)
			->execute(\Config::get('publicauth.db_connection'));
	...
	public function update_user($values, $username = null)
	{
		...
		$current_values = \DB::select_array(\Config::get('publicauth.table_columns', array('*')))
			->where('username', '=', $username)
			->from(\Config::get('publicauth.table_name'))
			->execute(\Config::get('publicauth.db_connection'));
		...
		$affected_rows = \DB::update(\Config::get('publicauth.table_name'))
			->set($update)
			->where('username', '=', $username)
			->execute(\Config::get('publicauth.db_connection'));
	...
	public function delete_user($username)
	{
		...
		$affected_rows = \DB::delete(\Config::get('publicauth.table_name'))
			->where('username', '=', $username)
			->execute(\Config::get('publicauth.db_connection'));
	...
	public function create_login_hash()
	{
		...
		//$login_hash = sha1(\Config::get('publicauth.login_hash_salt').$this->user['username'].$last_login);
		$login_hash = sha1(\Config::get('publicauth.login_hash_salt').$this->user['username']);

		\DB::update(\Config::get('publicauth.table_name'))
	...
	public function get_groups()
	{
		...
		return array(array('Publicgroup', $this->user['group']));
	}
	...
	{
		return \Config::get('publicauth.guest_login', true);
	}
}

// end of file publicauth.php
