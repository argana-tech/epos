...
class Controller_Index extends Controller_Common
{
	...
	{
		\Response::redirect('receive/subject/index');
	...
	public function action_downloadannounce()
	{
		...
		if(\Input::get('temp')) {
			$upload_file_path = \Config::get('upload_temp_file_path.announce');
		}
	...
	public function action_uploadslide() {
		try {
			...
			file_put_contents(\Config::get('update_error_log'), date('Y-m-d H:i:s') . " announce:" . $e->getMessage() . "\n", FILE_APPEND);
