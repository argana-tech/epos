...
class Controller_Common extends \Controller_Hybrid
{
	...
	{
		$res = array();
		$res['announce'] = $this->upload_file_by_name($id, 'announce');
				...
				try {
					...
					for($i=1;$i<=$slide_count;$i++) {
						...
						if(file_exists($upload_dir_path . "/" . $i . '.png')) {
							...
							$("#slide_' . $i . '", parent.document).attr("src", "' . \Uri::create('/announce/index/slideannounce?temp=1&file_name=' . $id . '/' . $i . '.png&uni=' . uniqid()) . '");
							// ]]></script>';

							continue;
						}

						$presentation->Slides[$i]->Export($upload_dir_path . "/" . $i . '.png', "png");
	...
	{
		$upload_file_path = \Config::get('upload_temp_file_path.announce') . $id . "/";
	...
	{
		$from_path = \Config::get('upload_file_path.announce');
		$to_path = \Config::get('upload_temp_file_path.announce');

		if($from_is_temp) {
			$from_path = \Config::get('upload_temp_file_path.announce');
			$to_path = \Config::get('upload_file_path.announce');
		}
