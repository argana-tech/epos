...
namespace Admin;
class Controller_Marker extends Controller_Common
{
	...
	public function action_index()
	{
                ...
                $this->template->content = \Response::forge(\ViewModel::forge('marker/index'));
	...
	public function action_new()
	{
		...
		$fields = $this->initActionFields();
                ...
                $this->template->content = \Response::forge(\ViewModel::forge('marker/new'));
	}
	...
	public function action_create()
	{
		if (\Input::method() != 'POST')
		{
			...
			return \Response::redirect('marker/index');
		}
		...
		if ($validation->run())
		{
			...
			if ($admin->save(null, true))
			{
				...
				foreach($subjects as $subject) {
					if(array_key_exists($subject->id, $marks)) {
						$input_mark = array(
									'user_id' => $admin->id,
						...
						$model_mark = \Model_Mark::forge($input_mark);
						$model_mark->save();
					}
				}

				\Session::set_flash('success', 'アカウントを保存しました。');
				return \Response::redirect('admin/marker/index');
			}
		}
	...
	public function action_edit($id)
	{
		...
		$viewmodel = \ViewModel::forge('marker/edit');
		$viewmodel->set('id', $id);
	...
	public function action_update()
	{
		...
		if ($validation->run())
		{
			...
			if ($admin->save(null, true))
			{
				...
				foreach($subjects as $subject) {
					$mark = \Model_Mark::find('first', array(
										    'where' => array(
